...
function insertData(req){   //往数据集中插入数据
  let res = {};
  if(req.type === 'square'){
    res = {
      'id': squareDB.filter(data => data.email === req.email).length + 1,
      'text': req.text,
      'imgSrc': req.imgSrc,
      'email': req.email,
      'userPhotoSrc': req.userPhotoSrc,
      'userName': req.userName,
      'starsNum': 0,
      'commentsNum': 0,
      'updateTime': getTime(),
      'userSign': req.userSign,
    }
    squareDB.unshift(res);    //越新的数据越靠前
    localStorage.setItem('squareDB', JSON.stringify({squareDB}));   //更新到本地存储
  }
    ...
    res = {
      ...
      'commentUserEmail': req.commentUserEmail,
      'commentUserPhoto': req.commentUserPhoto,
      'comment': req.comment,
    }
    commentsDB.unshift(res);    //越新的数据越靠前
    localStorage.setItem('commentsDB', JSON.stringify({commentsDB}));   //更新到本地存储

    //更新广场表中的评论数
    squareDB.filter(data => data.email === req.email && data.id === req.id).forEach(
      data => {data.commentsNum++}
    );
    localStorage.setItem('squareDB', JSON.stringify({squareDB}));   //更新到本地存储
  }
  return res;
...
//创建前端数据
let squareDB = [];    //广场数据表
if(!localStorage.getItem('squareDB')){
  for(let i = 1; i < 6; i++)
    squareDB.push({
        'id': i,
        'text': '打卡' + i,
  ...
  localStorage.setItem('squareDB', JSON.stringify({squareDB}));
}
else{
  squareDB = JSON.parse(localStorage.getItem('squareDB')).squareDB;
}

let commentsDB = [];    //评论数据表
if(!localStorage.getItem('commentsDB')){
  ...
  commentsDB = JSON.parse(localStorage.getItem('commentsDB')).commentsDB;
}